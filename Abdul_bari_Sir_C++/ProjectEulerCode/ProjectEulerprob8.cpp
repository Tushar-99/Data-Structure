//This solution takes 50 digits pairs so answer is diiferent
...
//     int A[20][50]={{7,3,1,6,7,1,7,6,5,3,1,3,3,0,6,2,4,9,1,9,2,2,5,1,1,9,6,7,4,4,2,6,5,7,4,7,4,2,3,5,5,3,4,9,1,9,4,9,3,4},
...
//                    {7,8,6,6,4,5,8,3,5,9,1,2,4,5,6,6,5,2,9,4,7,6,5,4,5,6,8,2,8,4,8,9,1,2,8,8,3,1,4,2,6,0,7,6,9,0,0,4,2},
...
//         for(int p = 0 ; p < 20 ; p++){
//             for(int a=0 , b=1 , c=2 ,d = 3,e = 4,f = 5, g = 6 , h = 7, i = 8, j = 9,k = 10 ,l = 11, m = 12; m < 50 ;
//             a++ , b++ ,c++ , d++ ,e++ ,f++ ,g++ ,h++ ,i++ ,j++, k++ ,l++ ,m++){
//                 max = A[p][a]*A[p][b]*A[p][c]*A[p][d]*A[p][e]*A[p][f]*A[p][g]*A[p][h]*A[p][i]*A[p][j]*A[p][k]*A[p][l]*A[p][m];
//                 opp.push_back(max);
//                 if(max > n){
//                     n=max;
//                 }
...
//         cout<<n<<endl;
//         // for(vector<int>::iterator i=opp.begin() ; i != opp.end() ; ++i){
...
//         int sizeopp=opp.size();
//         sort(opp.begin(),opp.end());
        
//         for(vector<long long>::iterator i=opp.begin() ; i != opp.end() ; ++i)
//             cout<<*i<<" ";
        
//         cout<<endl;
//         vector<long long>::iterator o=opp.end();
//         cout<<*o;
// }

// //Another solution but taking whole digits at a time not pair of 50 like above approach 
#include <iostream>
...
int main() {
    ...
    long long loopProduct = 1;
    long long largestProduct = 0;
    
    for (int i = 0; i <= 987; i++){
        loopProduct = 1;
            for (int j = 0; j <= 12; j++) 
                loopProduct = loopProduct * ((int)(numberSeries[i + j] - '0'));
                //Note Here '0' indicates 48 while converting string/character into int we have to aware of this ascii conversion

        if (loopProduct > largestProduct) {
            largestProduct = loopProduct;
        }
    }

...
float phool(float side1, float side2, float side3 ){
  float s = (side1+side2+side3)/2;
  float are = sqrt(s*(s-side1)*(s-side2)*(s-side3));
    return are;
}
 int main(){
   float u,x,b,c,d,va,vb,vc,vd; 
   cin>>u>>x>>b>>c>>d>>va>>vb>>vc>>vd;
  char da,db,dc,dd;
  cin>>da>>db>>dc>>dd;
  if(da=='D'){
    va=va*(-1);
  ...
  float abc[100];
  for(int i=0;i<100;i++){
    abc[i]=phool(ab[i],bc[i],ac[i]);
  }
  float adc[100];
  for(int i=0;i<100;i++){
    adc[i]=phool(ad[i],cd[i],ac[i]);
  }
  float abd[100];
  for(int i=0;i<100;i++){
    abd[i]=phool(ab[i],ad[i],bd[i]);
  }
  float bcd[100];
  for(int i=0;i<100;i++){
    bcd[i]=phool(bc[i],cd[i],bd[i]);
  }
